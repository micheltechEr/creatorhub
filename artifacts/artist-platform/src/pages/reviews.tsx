            ...
            <div className="flex-1 space-y-2.5">
              {ratingDistribution.map(({ stars, count, percentage }) => (
                <div key={stars} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-1.5 bg-muted overflow-hidden" style={{ borderRadius: "1px" }}>
                    <div
      ...
      {isLoading ? (
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-24 bg-muted border border-border animate-pulse" style={{ borderRadius: "4px" }} />
          ))}
        </div>
      ) : reviews.length === 0 ? (
        <div className="bg-card border border-border p-16 text-center" style={cardStyle}>
